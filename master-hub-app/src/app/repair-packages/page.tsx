...
function jsonToTsv(text:string){try{const raw=JSON.parse(text);const rows=Array.isArray(raw)?raw:Array.isArray(raw?.parts)?raw.parts:[];if(!rows.length||typeof rows[0]!=="object")return text;const keys=Array.from(new Set(rows.flatMap((r:any)=>Object.keys(r))));return [keys.join("\t"),...rows.map((r:any)=>keys.map(k=>String(r?.[k]??"")).join("\t"))].join("\n")}catch{return text}}
...
function parseRows(text:string,source="bulk import"):Part[]{const lines=text.split(/\r?\n/).map(x=>x.trim()).filter(Boolean);if(!lines.length)return[];const delimiter=lines.some(l=>l.includes("\t"))?"\t":",";const cells=lines.map(l=>parseDelimited(l,delimiter));const header=cells[0].map(x=>x.toLowerCase().replace(/[_-]+/g," ").trim());const find=(...names:string[])=>header.findIndex(h=>names.some(n=>h===n||h.includes(n)));const nameI=find("part name","description","item description","name"),numberI=find("part #","part#","part number","part no","partnum","pn","item number","product code"),compatI=find("compatible with","compatible","model","equipment","applies to","machine"),glsI=find("gls","gls code","gls number","gls #"),costI=find("standard cost","unit cost","part cost","dealer cost","cost"),glsCostI=find("gls cost","gls price","gls unit cost"),listI=find("list price","sell price","retail price","price"),sourceI=find("source","vendor","supplier","organization");const hasHeader=[nameI,numberI,compatI,glsI,costI,glsCostI,listI,sourceI].some(i=>i>=0);const data=hasHeader?cells.slice(1):cells;return data.map((r,i)=>({id:crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${i}`,name:r[nameI>=0?nameI:2]||r[0]||"",partNumber:r[numberI>=0?numberI:0]||"",compatibleWith:r[compatI>=0?compatI:2]||"",gls:glsI>=0?(r[glsI]||""):"",cost:costI>=0?num(r[costI]||"0"):0,glsCost:glsCostI>=0?num(r[glsCostI]||"0"):0,listPrice:listI>=0?num(r[listI]||"0"):0,source:sourceI>=0?(r[sourceI]||source):source})).filter(p=>p.name||p.partNumber||p.gls)}
...
export default function RepairPackagesPage(){
  ...
  const mergeImported=(rows:Part[],label:string)=>{if(!rows.length){flash("No parts detected");return}setLibrary(existing=>dedupeParts([...existing,...rows]));flash(`${rows.length} rows compiled from ${label} · duplicate parts merged · COST resolved from the parts database when available`)};
  ...
  const importFiles=async(e:ChangeEvent<HTMLInputElement>)=>{const files=Array.from(e.target.files||[]);if(!files.length)return;setImporting(true);let all:Part[]=[];let rejected:string[]=[];for(const file of files){const ext=file.name.toLowerCase();if(!/\.(csv|tsv|txt|md|json|html?)$/.test(ext)){rejected.push(file.name);continue}try{const text=await file.text();all=[...all,...parseRows(normalizeDocumentText(text,file.name),file.name)]}catch{rejected.push(file.name)}}if(all.length)mergeImported(all,`${files.length-rejected.length} document${files.length-rejected.length===1?"":"s"}`);if(rejected.length)flash(`Imported supported documents. Skipped unsupported: ${rejected.join(", ")}`);setImporting(false);e.target.value=""};
  ...
  return <main style={{minHeight:"100vh",background:"#080b12",color:"#f4f4f7",fontFamily:"Arial,sans-serif",padding:24}}><div style={{maxWidth:1500,margin:"0 auto",display:"grid",gap:16}}>
    ...
    <section style={{display:"grid",gridTemplateColumns:"minmax(320px,.75fr) minmax(0,1.6fr)",gap:16}}>
      <div style={panel}><h2 style={h2}>Parts Library Importer</h2><p style={muted}>Import one or many documents or paste data. Supported files: CSV, TSV, TXT, Markdown, JSON and HTML text exports. The importer detects Part Name, Part #, GLS, model/compatibility, Standard Cost, GLS Cost, List Price and source fields. Existing Part # / GLS records are merged instead of duplicated.</p><label style={{...secondary,display:"inline-block",marginBottom:10,cursor:"pointer"}}>{importing?"Importing…":"Import Documents"}<input type="file" multiple accept=".csv,.tsv,.txt,.md,.json,.html,.htm,text/csv,text/plain,application/json" onChange={importFiles} style={{display:"none"}} disabled={importing}/></label><textarea value={paste} onChange={e=>setPaste(e.target.value)} placeholder="Paste CSV, TSV, Markdown table, JSON, or exported document text" style={{...input,minHeight:180,resize:"vertical"}}/><button onClick={importText} style={{...primary,marginTop:10}}>Compile & Import Parts</button><p style={{...muted,marginBottom:0}}><strong style={{color:"#d8dce6"}}>COST rule:</strong> use GLS Cost first; if unavailable use imported Standard/Part Cost. A document that only supplies a Part # can still inherit its existing cost when it matches a part already in the database.</p></div>
      <div style={panel}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",marginBottom:10}}><h2 style={h2}>Unique Parts Database</h2><strong>{library.length} unique parts</strong></div><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search unique parts by name, PN, GLS, model or source" style={{...input,marginBottom:10}}/><div style={{overflow:"auto",maxHeight:430,border:"1px solid #252c39",borderRadius:10}}><table style={table}><thead><tr>{["PART NAME","PART #","GLS","COMPATIBLE WITH","COST","LIST",""].map(x=><th key={x} style={th}>{x}</th>)}</tr></thead><tbody>{filtered.map(p=><tr key={identity(p)}><td style={td}>{p.name}</td><td style={td}>{p.partNumber}</td><td style={td}>{p.gls||"—"}</td><td style={td}>{p.compatibleWith}</td><td style={{...td,textAlign:"right"}}>{effectiveCost(p)>0?money(effectiveCost(p)):"—"}</td><td style={{...td,textAlign:"right"}}>{p.listPrice>0?money(p.listPrice):"—"}</td><td style={td}><button onClick={()=>addToPackage(p)} style={mini}>Add</button></td></tr>)}</tbody></table></div></div>
    </section>
    ...
    {assistOpen&&<div style={{...panel,background:"#0d1320",marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><h3 style={{margin:"0 0 4px",fontSize:14}}>AI Assist · Complete Scoped Package</h3><p style={{...muted,margin:0}}>Uses verified model/job recipes against the Unique Parts Database. Every matched component inherits the database price; duplicate Part # / GLS entries collapse automatically.</p></div>{suggested.length>0&&<button onClick={addKnownRecipe} style={primary}>Add all {suggested.length} matched parts</button>}</div>
      {knownRecipe.length>0?<div style={{marginTop:12,overflow:"auto",border:"1px solid #252c39",borderRadius:10}}><table style={table}><thead><tr>{["SCOPE","PART / COMPONENT","DN PN","STATUS","COST","NOTE"].map(x=><th key={x} style={th}>{x}</th>)}</tr></thead><tbody>{recipeRows.map(({recipe,part})=><tr key={recipe.partNumber}><td style={td}>{recipe.scope}</td><td style={td}>{recipe.name}</td><td style={td}>{recipe.partNumber}</td><td style={{...td,color:part?"#67d6ba":"#e6b767"}}>{part?"Matched in Unique Parts":"Missing from Unique Parts"}</td><td style={{...td,textAlign:"right"}}>{part&&effectiveCost(part)>0?money(effectiveCost(part)):"—"}</td><td style={td}>{recipe.note||""}</td></tr>)}</tbody></table></div>:<p style={{...muted,marginTop:12}}>Enter a recognized model/job package. Example: <strong>BEAM Audio Green Box Update</strong>. Unknown job types are not auto-filled until a verified recipe is added.</p>}
      {missingRecipe.length>0&&<p style={{...muted,color:"#e6b767",marginTop:10}}>{missingRecipe.length} recipe part{missingRecipe.length===1?"":"s"} are not yet in the Unique Parts Database. Import them to make the package fully costed.</p>}
    </div>}

    <div style={{overflow:"auto",border:"1px solid #252c39",borderRadius:10}}><table style={table}><thead><tr>{["PART NAME","PART #","GLS","COMPATIBLE WITH","COST","LIST PRICE",""].map(x=><th key={x} style={th}>{x}</th>)}</tr></thead><tbody>{dedupeParts(selected).map(p=><tr key={identity(p)}><td style={td}>{p.name}</td><td style={td}>{p.partNumber}</td><td style={td}>{p.gls||"—"}</td><td style={td}>{p.compatibleWith}</td><td style={{...td,textAlign:"right"}}>{effectiveCost(p)>0?money(effectiveCost(p)):"—"}</td><td style={{...td,textAlign:"right"}}>{p.listPrice>0?money(p.listPrice):"—"}</td><td style={td}><button onClick={()=>setSelected(x=>x.filter(q=>identity(q)!==identity(p)))} style={mini}>Remove</button></td></tr>)}{dedupeParts(selected).length===0&&<tr><td colSpan={7} style={{...td,textAlign:"center",color:"#7d8596",padding:28}}>No parts in this package</td></tr>}</tbody><tfoot><tr><td colSpan={4} style={{...td,textAlign:"right",fontWeight:800}}>RUNNING TOTAL</td><td style={{...td,textAlign:"right",fontWeight:800}}>{money(totalCost)}</td><td style={{...td,textAlign:"right",fontWeight:800}}>{money(totalList)}</td><td style={td}/></tr></tfoot></table></div></section>
